...
{
    public partial class SiteComparisonChart : UserControl
    {
        private const string FaoSiteComparisonGraphInfo = "FaoSiteComparisonGraphInfo";
        private const string FaoSiteComparisonHiddenSeries = "FaoSiteComparisonHiddenSeries";

        private GraphData GraphInformation
        {
            get
            {
                return (GraphData)Session[FaoSiteComparisonGraphInfo];
            }
            set { Session[FaoSiteComparisonGraphInfo] = value; }
        }

        public List<string> ShowSeries
        {
            set { Session[FaoSiteComparisonHiddenSeries] = value; }
            get { return (List<string>)Session[FaoSiteComparisonHiddenSeries]; }
        }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            {
                chrtSiteComparison.Visible = true;
                BuildLineChart();
            }
            else
            {
                chrtSiteComparison.Visible = false;
            }
        ...
        public void LoadChart(List<SiteComparisonEntity> comparisonData)
        {
            var series = comparisonData.Select(d => d.Location).Distinct().ToList();
            ...
            do
            {
                foreach (var stg in seriesToGraph)
                {
                    ...
                    var dataEntry = comparisonData.FirstOrDefault(d => d.Location == stg.SeriesName
                                                                       && d.Year == currentYear
                                                                       && d.Week == currentWeek);
                    int yVal = 0;
                    if (dataEntry != null)
        ...
        private void BuildLineChart()
        {
            ...
            chrtSiteComparison.ChartAreas.Add(ca);
            ...
            foreach (var cd in GraphInformation.SeriesData)
            {
                ...
                chrtSiteComparison.Series.Add(cs);
                ...
                chrtSiteComparison.Legends["RightLegend"].CustomItems.Add(legendItem);
            ...
            chrtSiteComparison.Legends["SlideLegend"].CustomItems.Add(slideLegendItem);
        }

        protected void chrtSiteComparison_Click(object sender, ImageMapEventArgs e)
        {
            ...
            {
                var legend = chrtSiteComparison.Legends["Legend1"];
            ...
            {
                ShowLegend = false;
                chrtSiteComparison.Legends["RightLegend"].Enabled = false;
                return;
            }
            if (e.PostBackValue.StartsWith("ShowLegend"))
            {
                ShowLegend = true;
                chrtSiteComparison.Legends["RightLegend"].Enabled = true;
                return;
            }
        }
