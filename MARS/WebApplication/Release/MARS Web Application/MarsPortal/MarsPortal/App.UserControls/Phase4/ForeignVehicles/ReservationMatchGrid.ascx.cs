...
{
    public partial class ReservationMatchGrid : UserControl
    {
        private const string SessionGridData = "ReservationMatchGridSessionData";

        private const string ReservationMatchGridSortDirectionSessionString = "ReservationMatchGridSortDirectionSessionString";
        private const string ReservationMatchGridSortColumnSessionString = "ReservationMatchGridSortColumnSessionString";

        public List<ReservationMatchGridRow> GridData
        {
            private get { return Session[SessionGridData] as List<ReservationMatchGridRow>; }
            set { Session[SessionGridData] = value; }
        }

        private SortDirection OverviewSortDirection
        {
            get { return (SortDirection)Session[ReservationMatchGridSortDirectionSessionString]; }
            set { Session[ReservationMatchGridSortDirectionSessionString] = value; }
        }

        private PropertyInfo OverviewSortColumn
        {
            get { return Session[ReservationMatchGridSortColumnSessionString] as PropertyInfo; }
            set { Session[ReservationMatchGridSortColumnSessionString] = value; }
        }
        ...
        public int HighlightedReservationId
        {
            get { return int.Parse(hfHighlightedReservationId.Value); }
            set { hfHighlightedReservationId.Value = value.ToString(); }
        }
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            lblHeader.Text = ShowSelectColumn ? "Reservations" : "Matching Reservations";

            foreach (GridViewRow gr in gvOverview.Rows)
            {
                if (gr.Cells[9].Text == string.Empty) continue;
                var s = int.Parse(gr.Cells[9].Text);
                if (HighlightedReservationId == s)
                {
        ...
        {
            if (e.CommandName == "ShowVehicles")
            {
                HighlightedReservationId = int.Parse(e.CommandArgument.ToString());
            }
        ...
        {
            OverviewSortColumn = typeof(ReservationMatchGridRow).GetProperty(e.SortExpression);
