...
{
    public class FleetComparisonRepository : IComparisonRepository
    {
        ...
        FeaFleetQueryable _feaFleetQ;
        PoolingCheckInReservations _resFleetCIQ;
        PoolingCheckOutReservationsWithLabels _resFleetCOQ;
        private int DEFAULTVALUE = 0, FLAG = -999999;
        ...
        IList<DayActualEntity> GetPoolingCIData(Enums.DayActualTime tme)
        {
            using (PoolingDataClassesDataContext db = new PoolingDataClassesDataContext())
            {
                ...
                try
                {
                    ...
                    l = _resFleetCIQ.GetFleetReservationsWithLabels(q2And3, Filter, tme, db).ToList();
                }
                catch (SqlException ex)
                {
                 //   ILog _logger = log4net.LogManager.GetLogger("Pooling");
