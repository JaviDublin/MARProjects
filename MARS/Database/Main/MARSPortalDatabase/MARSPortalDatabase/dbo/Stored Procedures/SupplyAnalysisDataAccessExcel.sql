...
CREATE PROCEDURE [dbo].[SupplyAnalysisDataAccessExcel] 
...
begin
	select 
		REP_DATE
		, ExpectedFleet - NecessaryConstrained as ConstrainedSupply
		, ExpectedFleet - NecessaryUnConstrained as UnConstrainedSupply
		, ExpectedFleet - NecessaryBooked as BookedSupply
		, country
		, Car_Segment
		, Car_Class
		, Car_Group
...
begin
	select GETDATE() as Rep_Date
		, MIN(ExpectedFleet - NecessaryConstrained) as ConstrainedSupply
		, MIN(ExpectedFleet - NecessaryUnConstrained) as UnConstrainedSupply
		, MIN(ExpectedFleet - NecessaryBooked) as BookedSupply
		, country
		, Car_Segment
		, Car_Class
		, Car_Group
		...
		, CMS_Location_Group
		from #Returned
		group by DATEPART(iso_week, REP_DATE), YEAR(Rep_Date), country, Car_Segment, Car_Class, Car_Group, CMS_Pool, CMS_Location_Group
		order by country,rep_date
end
