...
(
	...
	select
	VehicleId,OwningCountry,OwningArea,
	DaysInOwningArea,OwningLocationCode,Vin,LicensePlate,UnitNumber,TasModelCode,ModelGroup,ModelDescription,
	ManufacturerCode,VisionManufacturerCode,VisionModelCode,CarGroup,LastArea,LastLocationCode,LastChangeDateTime,
	LastDocumentNumber,LastMilage,LastDriverName,DaysInCountry,DaysInArea,DaysInLocation,DaysSinceLastMovement,PreviousLocationCode,
	DaysSinceLastRevenueMovement,LastRevenueMovementDate,LastRevenueMovementLocationCode,LastRevenueMovementStatus,OnRentOrIdleIndicator,
	LastOperationalStatusId,DaysInCurrentOperationalStatus,LastDocumentType,LastDocumentStatus,LastMovementTypeId,ExpectedArea,
	ExpectedLocationCode,ExpectedDateTime,CarGroupCharged,InstallationDate,InstallationDeliveryDate,InstallationMsoDate,
	DaysInService,FirstRevenueMovementDate,FirstRevenueMovementLocationCode,InstallationModelYear,InstallationVendorNumber,InstallationPoNumber,
	SpiCode,DepreciationStatus,VisionClassCode,VisionTypeCode,HoldFlag1,HoldFlag2,HoldFlag3,CapitalCost,BaseAndOptions,
	DepreciationAmount,BookValue,Colour,DaysInMm,DaysInBd,ReturnTime,DaysUntilBlockDate,BlockDate,
	BlockWarningMileage,BlockMileage,SaleProcessDate,SaleDate,VehicleFleetTypeId,IsFleet,IsNonRev,Comment,LastLocationId
	from Vehicle
	where IsFleet = 1
)
	MERGE into Target
	USING ( 
			select fea.*, vft.VehicleFleetTypeId
					...
					, mt.[MovementTypeId]
					, l.dim_Location_id as LocationId
			...
			join Locations l on fea.LstWWD = l.Location
			)
		as Source
	...
	WHEN MATCHED THEN
		UPDATE SET Target.IsFleet = Source.IsFleet
					...
					, Target.SaleProcessDate = Source.SPROCDAT, Target.SaleDate = Source.SDATE, Target.LastLocationId = Source.LocationId
					 
	WHEN NOT MATCHED BY Target and (Source.IsFleet = 1) THEN
		INSERT ( OwningCountry, OwningArea, DaysInOwningArea, OwningLocationCode, Vin, LicensePlate, UnitNumber, TasModelCode
				...
				, DepreciationAmount, BookValue, Colour, DaysInMm, DaysInBd, ReturnTime, DaysUntilBlockDate, BlockDate
				, BlockWarningMileage, BlockMileage, SaleProcessDate, SaleDate, VehicleFleetTypeId, IsFleet, IsNonRev, LastLocationId)
		VALUES (COUNTRY, OWNAREA, null, OWNWWD, SERIAL, LICENSE, UNIT, MODEL, MODGROUP, MODDESC, null, null, VISMODEL, VC
				...
				, VehicleFleetTypeId, IsFleet, 0, LocationId)
	...
	update vehicle
	set LastDriverName = REPLACE(LastDriverName, ',', ' ')
	where LastDriverName like '%,%'
		and IsFleet = 1
END