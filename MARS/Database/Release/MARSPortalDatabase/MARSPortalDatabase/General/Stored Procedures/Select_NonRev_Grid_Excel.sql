...
-- =============================================
CREATE PROCEDURE [General].[Select_NonRev_Grid_Excel]
...
BEGIN
	...
	BEGIN
		...
		(
			Lstwwd , CarGroup , Plate , Unit , 
			ModelDesc , LstDate , LstMlg ,
			NRDays , OperStat , MoveType , DepStat ,
			Carhold , NextRent , RemarkCode , Remark , Serial, DueDate, DueWwd
		)
		
		
		SELECT
			FNR.Lstwwd , FNR.CarGroup , FNR.Plate , FNR.Unit , 
			FNR.ModelDesc , FNR.LstDate , FNR.LstMlg ,
			FNR.NRDays , FNR.OperStat , 
			FNR.MoveType , FNR.DepStat , FNR.Carhold , FNR.ERDate as [NextRent] ,
			NRRL.RemarkText			AS [RemarkCode], 
			FNR.Remark				AS [Remark],
			FNR.Serial,
			FNR.DueDate as DueDate,
			FNR.Duewwd as DueWwd
		FROM 
	...
	SELECT 
		CS.Lstwwd , CS.CarGroup , CS.Plate , CS.Unit ,
