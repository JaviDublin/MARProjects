...
-- =============================================
CREATE PROCEDURE [General].[Select_NonRev_Chart_Comparison]
...
BEGIN
	...
	SELECT 
		...
		AVG(t.TotalFleetNR)/@rowcount	AS [TotalFleetNR],
...
(
	...
	SET @totaFleet			= (SELECT SUM(Fleet) FROM @RESULT)
	
	SET @totalFleetNonRev	= (SELECT SUM(NonRev) FROM @RESULT)
	...
	BEGIN
		SELECT 
			CS.GroupLevel ,
			CS.Fleet as [TotalVehicles], 
			CS.NonRev as [TotalVehiclesNR] 
		FROM 
			@RESULT CS
	...
	BEGIN
		SELECT 
			CS.GroupLevel ,
			CONVERT(FLOAT,REPLACE(CS.PctFleet,' %','')) as [TotalVehicles] ,
			CONVERT(FLOAT,REPLACE(CS.PctNonRev,' %','')) as [TotalVehiclesNR]
		FROM 
			@RESULT CS
