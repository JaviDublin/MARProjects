...
-- =============================================
CREATE PROCEDURE [General].[Select_NonRev_Chart_HistoricalTrend]
...
BEGIN
	...
	IF @dateRange = 'Custom'			BEGIN SET @day_diff = 0		END
...
WHEN @fleet_name = 'LICENSEE'		 THEN SUM(case when Dr.OperStat = 'UNK' then Dr.TotalFleetNR_Licensee else null end) 
	...
	WHERE 
		...
		--Dr.dim_calendar_id between @start_calendar_id and @end_calendar_id
	...
	SELECT 
		CONVERT(varchar(11),CS.Rep_Date,103) as [Rep_Date],
		CS.Total_Fleet_NR , 
