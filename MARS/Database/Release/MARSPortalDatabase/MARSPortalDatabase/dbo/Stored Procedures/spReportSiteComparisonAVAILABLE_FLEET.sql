...
-- =============================================
CREATE procedure [dbo].[spReportSiteComparisonAVAILABLE_FLEET] 
...
BEGIN
	...
	CREATE TABLE #TMP_LOCATIONS(
		...
		TOPIC_COUNTER	INT,	--TOPIC: AVAILABLE_FLEET
	...
	SELECT FES.REP_DATE, FES.COUNTRY, FES.LOCATION, SUM(FES.AVAILABLE_FLEET), SUM(FES.OPERATIONAL_FLEET) 
	FROM FLEET_EUROPE_SUMMARY_HISTORY AS FES
	WHERE
	...
	GROUP BY FES.REP_DATE, FES.LOCATION, FES.COUNTRY
