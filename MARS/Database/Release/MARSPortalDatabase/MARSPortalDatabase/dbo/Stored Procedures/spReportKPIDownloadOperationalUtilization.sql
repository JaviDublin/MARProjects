...
CREATE procedure [dbo].[spReportKPIDownloadOperationalUtilization]
...
--===========================================================
	...
	(
		REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DAY_OF_WEEK, REP_DATE, ON_RENT, OPERATIONAL_FLEET
	)
	SELECT 
		d.REP_YEAR, d.REP_MONTH, d.Rep_WeekOfYear , d.Rep_DayOfWeek , DATEADD(d, 0, DATEDIFF(d, 0, d.REP_DATE)), 
		SUM(ON_RENT), SUM(OPERATIONAL_FLEET) 
	FROM 
	...
	SELECT @entity, @key1, 'Insert KPI Download Operational Utilization', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')	
	
-- Select Data
--===========================================================

	SELECT 
		REP_DATE,(ON_RENT/OPERATIONAL_FLEET) AS [OPS_UTI], OPERATIONAL_FLEET AS [OPS_FLEET]
	FROM 
		#TMP_TREND 
	ORDER BY 
		REP_DATE
