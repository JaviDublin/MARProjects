CREATE procedure [dbo].[spReportKPIIdleFleet]
...
--===========================================================
	...
	BEGIN
		...
		(
			...
			REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, ON_RENT, TOTAL_FLEET
		)
		SELECT 
			FES.dim_calendar_id, 
			NumDays = 1 ,
		...
		SELECT @entity, @key1, 'Insert KPI Idle Fleet', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')	
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		IF (@grouping_criteria = 'DAY')
			...
			BEGIN
				...
				BEGIN
					...
					AVG(TOTAL_FLEET - ON_RENT) AS [KPI]
					FROM 
						#TMP_TREND 
					GROUP BY 
		...
		IF (@grouping_criteria = 'DAY')
			BEGIN
				SELECT 
					REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE,
				CASE
					 WHEN TOTAL_FLEET = 0 THEN 0
					 ELSE ((TOTAL_FLEET - ON_RENT) * 100.0) / TOTAL_FLEET
				END AS [KPI]
				FROM 
					#TMP_TREND 
				ORDER BY 
			...
			BEGIN
				...
				BEGIN
					SELECT 
						...
						REP_WEEK_OF_YEAR AS [REP_WEEK_OF_YEAR],
					CASE
						WHEN AVG(TOTAL_FLEET) = 0 THEN 0
						ELSE (AVG(TOTAL_FLEET - ON_RENT)*100.0) / AVG(TOTAL_FLEET)
					END AS [KPI]
					FROM 
						#TMP_TREND 
					GROUP BY 
