...
CREATE procedure [dbo].[spReportSiteComparisonTN]
...
declare @EndDateStart datetime
 ...
 SELECT 
  FES.dim_Calendar_id, FES.COUNTRY, l.LOCATION, SUM(FES.TN), SUM(FES.TOTAL_FLEET), 1, 'd' 
 FROM 
 ...
 begin
  INSERT INTO #TMP_LOCATIONS(dim_calendar_id, COUNTRY, LOCATION, TOPIC_COUNTER, GROUP_COUNTER, NumDays, rowtype)
  SELECT 
   FES.dim_Calendar_id_start, FES.COUNTRY, l.LOCATION, SUM(FES.TN), SUM(FES.TOTAL_FLEET), max(FES.NumDays), 'm' 
  FROM 
 ...
 begin
  INSERT INTO #TMP_LOCATIONS(dim_calendar_id, COUNTRY, LOCATION, TOPIC_COUNTER, GROUP_COUNTER, NumDays, rowtype)
  SELECT 
   FES.dim_Calendar_id, FES.COUNTRY, l.LOCATION, SUM(FES.TN), SUM(FES.TOTAL_FLEET), max(FES.NumDays), 'm' 
  FROM 
