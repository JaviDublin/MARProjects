...
BEGIN
	...
	USING (
			select rs.EFF_DTTM, rs.RES_ID_NBR, rs.COUNTRY
				, CarGroup.car_group_id as CarGroupId
				, dbo.fnResGetUpgradedCarGroupId(rs.GR_INCL_GOLDUPGR, rs.N1TYPE, rs.COUNTRY) as UpgradedCarGroup
				...
				,[TACO], [FLIGHT_NBR], [GS], [N1TYPE], DATE_SOLD_DT, DATE_SOLD_TM, GR_INCL_GOLDUPGR
			...
			left join 
			(
				select cg.car_group, cs.country, cg.car_group_id
					, 0 as UpgradedCarGroupId
				from CAR_GROUPS cg
				join CAR_CLASSES cc on cg.car_class_id = cc.car_class_id
				join CAR_SEGMENTS cs on cc.car_segment_id = cs.car_segment_id
			) CarGroup on left(rs.GR_INCL_GOLDUPGR,2) = CarGroup.car_group and rs.COUNTRY = CarGroup.country
	...
	WHEN MATCHED THEN UPDATE SET
		...
		, r.[GR_INCL_GOLDUPGR] = UpgradedCarGroup
...
;
	...
	where RES_ID_NBR in
	(select RES_ID_NBR from ReservationStaging2 
		where  lower(RESV_BOOK_ACTN_CD) in('x', 'r') or lower(RESV_BOOK_CUR_ACTN_CD) in('x', 'r') )
