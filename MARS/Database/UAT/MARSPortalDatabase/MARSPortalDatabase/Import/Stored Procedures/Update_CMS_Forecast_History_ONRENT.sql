...
-- =============================================
CREATE PROCEDURE [Import].[Update_CMS_Forecast_History_ONRENT]
...
BEGIN
	...
	FROM 
		MARS_CMS_FORECAST_HISTORY FCH
	INNER JOIN
		(
			SELECT 
				FES.REP_DATE, FES.COUNTRY, L.CMS_LOCATION_GROUP_ID, CC.CAR_CLASS_ID, 
				SUM(FES.ON_RENT_MAX_ABSOLUTE)	AS 'ON_RENT', 
			...
			/*
			SELECT fh.Timestamp, l.cms_location_group_id, fh.CarGroupId
				, cast( sum(fh.MaxOnRent) as numeric(9,2)) as OnRent
				, cast( sum(fh.AvgAvailableFleet)as numeric(9,2)) as Available
				, cast( sum(fh.AvgOperationalFleet)as numeric(9,2)) as Operational
				, cast( sum(fh.AvgTotal)as numeric(9,2)) as Total
		...
		ON FCH.REP_DATE = FES.REP_DATE AND FCH.CMS_LOCATION_GROUP_ID = FES.CMS_LOCATION_GROUP_ID AND FCH.CAR_CLASS_ID = FES.CAR_CLASS_ID
	...
	FROM MARS_CMS_FORECAST_HISTORY FCH
		LEFT JOIN dbo.FleetPlanSummary(NULL, 1, @theDay, @theDay) FP ON FCH.REP_DATE = FP.targetDate AND FCH.CMS_LOCATION_GROUP_ID = FP.LOCATION_GROUP_ID AND FCH.CAR_CLASS_ID = FP.CAR_CLASS_ID
	...
	FROM MARS_CMS_FORECAST_HISTORY FCH
		...
		GROUP BY FESH.REP_DATE, FESH.COUNTRY, L.CMS_LOCATION_GROUP_ID, CC.CAR_CLASS_ID
			...
			SELECT fh.Timestamp as REP_DATE
				, l.cms_location_group_id
				, fh.CarGroupId as CAR_CLASS_ID
				, cast( sum(fh.PeakOnRent) as numeric(9,2)) as ON_RENT
