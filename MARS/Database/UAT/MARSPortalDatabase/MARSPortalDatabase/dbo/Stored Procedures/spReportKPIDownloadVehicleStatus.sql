...
CREATE procedure [dbo].[spReportKPIDownloadVehicleStatus]
...
--===========================================================
	...
	(
		...
		ON_RENT						INT,
		OVERDUE						INT,
		PREDELIVERY_GOLD			INT,
		IDLE						INT,
		UNAVAILABLE_OPS_FLEET		INT,
		UNAVAILABLE_NON_OPS_FLEET	INT
	)
...
--===========================================================
	...
	(
		...
		TOTAL_FLEET, ON_RENT, OVERDUE, PREDELIVERY_GOLD, IDLE, UNAVAILABLE_OPS_FLEET, 
		UNAVAILABLE_NON_OPS_FLEET
	)
	...
	SELECT 
		...
		SUM(PREDELIVERY + GOLD) AS 'PREDELIVERY_GOLD', 
		SUM(RT + SU)			AS 'IDLE', 
		SUM(BD + MM + TW + FS + RL + TN +
		(CASE
				WHEN
					(COUNTRY = 'UK')			
				THEN
	 ...
	 + WS +
		...
		(CASE
				...
				ELSE
					0
			END)) AS 'UNAVAILABLE_NON_OPS_FLEET'
	FROM 
	...
	SELECT @entity, @key1, 'Insert KPI Fleet Status', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')	
...
--===========================================================
	...
	SELECT 
		REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DAY_OF_WEEK, REP_DATE, 
		(ON_RENT * 100.0) / TOTAL_FLEET					AS [ON RENT], 
		(OVERDUE*100.0)/TOTAL_FLEET						AS [OVERDUE], 
		(PREDELIVERY_GOLD*100.0)/TOTAL_FLEET			AS [PREDELIVERY & GOLD], 
		(IDLE*100.0)/TOTAL_FLEET						AS [IDLE], 
		(UNAVAILABLE_OPS_FLEET*100.0)/TOTAL_FLEET		AS [UNAVAILABLE OPS FLEET], 
		(UNAVAILABLE_NON_OPS_FLEET*100.0)/TOTAL_FLEET	AS [UNAVAILABLE NON-OPS FLEET],
		'WK' + REP_WEEK_OF_YEAR + ' - ' + (CASE REP_DAY_OF_WEEK 
												WHEN 1 THEN 'MO'
												WHEN 2 THEN 'TU'
												WHEN 3 THEN 'WE'
												WHEN 4 THEN 'TH'
	...
	FROM 
		#TMP_TREND 
	ORDER BY 
		REP_YEAR, REP_MONTH, REP_DAY_OF_WEEK, REP_WEEK_OF_YEAR
