...
-- =============================================
CREATE procedure [dbo].[spReportKPIFleetStatus]
...
--===========================================================	
	...
	(
		...
		REP_DATE							DATETIME	,	
		TOTAL_FLEET							INT			,
		UNAVAILABLE_NON_OPERATIONAL_FLEET	INT			,
		UNAVAILABLE_OPERATIONAL_FLEET		INT			,
		AVAILABLE_FLEET						INT			,
		ON_RENT								INT		
	)
...
--===========================================================
	...
	BEGIN
		...
		(	dim_calendar_id, NumDays , rowtype,
			...
			TOTAL_FLEET, UNAVAILABLE_NON_OPERATIONAL_FLEET, 
			UNAVAILABLE_OPERATIONAL_FLEET, AVAILABLE_FLEET, ON_RENT
		)
		...
		SELECT 
			...
			SUM(CARSALES + CU + HA + HL + LL + NC + PL + TC)	AS 'UNAVAILABLE_NON_OPERATIONAL_FLEET', 
			SUM(BD + MM + TW + TB + WS + FS + RL + RP + TN)		AS 'UNAVAILABLE_OPERATIONAL_FLEET', 
			SUM(RT + SU + GOLD + PREDELIVERY + OVERDUE)			AS 'AVAILABLE_FLEET', 
		...
		SELECT @entity, @key1, 'Insert KPI Fleet Status', 'updated=' + 
			COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')	
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		SELECT 
			...
			SUM(CARSALES + CU + HA + HL + LL + NC + PL + TC)/MAX(NumDays)	AS 'UNAVAILABLE_NON_OPERATIONAL_FLEET', 
			SUM(BD + MM + TW + TB + WS + FS + RL + RP + TN)/MAX(NumDays)	AS 'UNAVAILABLE_OPERATIONAL_FLEET', 
			SUM(RT + SU + GOLD + PREDELIVERY + OVERDUE)/MAX(NumDays)		AS 'AVAILABLE_FLEET', 
			SUM(ON_RENT)/MAX(NumDays)										AS 'ON_RENT'
		FROM 
		...
		BEGIN
			...
			BEGIN
				SELECT 
					REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, 
					TOTAL_FLEET, 
					UNAVAILABLE_NON_OPERATIONAL_FLEET, 
					UNAVAILABLE_OPERATIONAL_FLEET, 
					AVAILABLE_FLEET, 
					ON_RENT
				FROM 
					#TMP_TREND 
			...
			BEGIN
				...
				BEGIN
					SELECT 
						...
						AVG(UNAVAILABLE_NON_OPERATIONAL_FLEET) AS [UNAVAILABLE_NON_OPERATIONAL_FLEET], 
						AVG(UNAVAILABLE_OPERATIONAL_FLEET) AS [UNAVAILABLE_OPERATIONAL_FLEET], 
		...
		BEGIN
			...
			BEGIN
				SELECT 
					...
					UNAVAILABLE_NON_OPERATIONAL_FLEET * 100.0 / TOTAL_FLEET AS [UNAVAILABLE_NON_OPERATIONAL_FLEET], 
					UNAVAILABLE_OPERATIONAL_FLEET * 100.0 / TOTAL_FLEET  AS [UNAVAILABLE_OPERATIONAL_FLEET], 
					AVAILABLE_FLEET * 100.0 / TOTAL_FLEET AS [AVAILABLE_FLEET], 
					ON_RENT * 100.0 / TOTAL_FLEET AS [ON_RENT]
				FROM 
					#TMP_TREND
				GROUP BY
					REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, 
			...
			BEGIN
				...
				BEGIN
					SELECT 
						...
						REP_WEEK_OF_YEAR AS [REP_WEEK_OF_YEAR],
						100.0 AS 'TOTAL_FLEET', 
						AVG(UNAVAILABLE_NON_OPERATIONAL_FLEET) * 100.0 / AVG(TOTAL_FLEET) AS [UNAVAILABLE_NON_OPERATIONAL_FLEET], 
						AVG(UNAVAILABLE_OPERATIONAL_FLEET) * 100.0 / AVG(TOTAL_FLEET) AS [UNAVAILABLE_OPERATIONAL_FLEET], 
						AVG(AVAILABLE_FLEET) * 100.0 / AVG(TOTAL_FLEET) AS [AVAILABLE_FLEET], 
						AVG(ON_RENT) * 100.0 / AVG(TOTAL_FLEET) AS [ON_RENT]
					FROM 
						#TMP_TREND 
					GROUP BY 
						REP_YEAR, REP_WEEK_OF_YEAR , SUBSTRING(CONVERT(VARCHAR(8),dim_calendar_id),5,2)
