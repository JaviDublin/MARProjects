...
CREATE procedure [dbo].[spReportSiteComparisonWS_NONRAC]
...
/*
exec spReportSiteComparisonWS_NONRAC
@start_date = '20110120' ,
@end_date = '20120410' ,
@select_by = 'VALUE' ,
...
declare @data2 varchar(max)
 ...
 CREATE TABLE #TMP_LOCATIONS
  ...
  TOPIC_COUNTER INT, --TOPIC: WS_NONRAC
...
declare @EndDateStart datetime
 ...
 SELECT 
  FES.dim_Calendar_id, FES.COUNTRY, l.LOCATION, SUM(FES.WS_NONRAC), SUM(FES.TOTAL_FLEET), 1, 'd' 
 FROM 
 ...
 begin
  INSERT INTO #TMP_LOCATIONS(dim_calendar_id, COUNTRY, LOCATION, TOPIC_COUNTER, GROUP_COUNTER, NumDays, rowtype)
  SELECT 
   FES.dim_Calendar_id_start, FES.COUNTRY, l.LOCATION, SUM(FES.WS_NONRAC), SUM(FES.TOTAL_FLEET), max(FES.NumDays), 'm' 
  FROM 
 ...
 begin
  INSERT INTO #TMP_LOCATIONS(dim_calendar_id, COUNTRY, LOCATION, TOPIC_COUNTER, GROUP_COUNTER, NumDays, rowtype)
  SELECT 
   FES.dim_Calendar_id, FES.COUNTRY, l.LOCATION, SUM(FES.WS_NONRAC), SUM(FES.TOTAL_FLEET), max(FES.NumDays), 'm' 
  FROM 
