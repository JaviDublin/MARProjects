...
-- =============================================
CREATE procedure [dbo].[spReportKPIOverdues]
...
--===========================================================
	...
	BEGIN
		...
		(
			...
			REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, OVERDUE, AVAILABLE_FLEET
		)
		...
		SELECT @entity, @key1, 'Insert KPI Overdues', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')	
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		BEGIN
			...
			BEGIN
				SELECT 
					...
					REP_DATE,
				CASE
					 WHEN AVAILABLE_FLEET = 0 THEN 0
					 ELSE (OVERDUE * 100.0) / AVAILABLE_FLEET
				END AS [KPI]
				FROM #TMP_TREND ORDER BY REP_DATE
			END
			ELSE 
			BEGIN
				IF (@grouping_criteria = 'WEEK')
				BEGIN
					SELECT 
						...
						REP_WEEK_OF_YEAR AS [REP_WEEK_OF_YEAR],
						CASE
							WHEN AVG(AVAILABLE_FLEET) = 0 THEN 0
							ELSE (AVG(OVERDUE)*100.0) / AVG(AVAILABLE_FLEET)
						END AS [KPI]
					FROM 
						#TMP_TREND 
					GROUP BY 
