...
-- =============================================
CREATE procedure [dbo].[spReportKPIUnavailableNonOperationalFleet]
...
--=====================================================================================	
	...
	BEGIN
		...
		(
			...
			REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, CARSALES, CARHOLD, 
			CU, HA, HL, LL, NC, PL, TC, WS , TOTAL_FLEET
		)
		...
		SELECT 
			...
			SUM(CARHOLD_H + CARHOLD_L) AS 'CARHOLD', 
			SUM(CU) AS 'CU', 
			SUM(HA) AS 'HA', 
			SUM(HL) AS 'HL', 
			SUM(LL) AS 'LL', 
		...
		SELECT @entity, @key1, 'Insert KPI Unavailable Non Operational Fleet', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')		
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		SELECT 
			...
			SUM(CARSALES) AS 'CARSALES', 
			SUM(CARHOLD_H + CARHOLD_L) AS 'CARHOLD', 
			SUM(CU)/MAX(NumDays) AS 'CU', 
			SUM(HA)/MAX(NumDays) AS 'HA', 
			SUM(HL)/MAX(NumDays) AS 'HL', 
			SUM(LL)/MAX(NumDays) AS 'LL', 
			SUM(NC)/MAX(NumDays) AS 'NC', 
			SUM(PL)/MAX(NumDays) AS 'PL', 
			SUM(TC)/MAX(NumDays) AS 'TC', 
			SUM(WS_NONRAC)/MAX(NumDays) AS 'WS' ,
	...
	BEGIN
		...
		BEGIN
			SELECT 
				...
				REP_WEEK_OF_YEAR, 
				REP_DATE, 
				CARSALES * 100.0 / TOTAL_FLEET AS [CARSALES], 
				CARHOLD * 100.0 / TOTAL_FLEET AS [CARHOLD], 
				CU * 100.0 / TOTAL_FLEET AS [CU], 
				HA * 100.0 / TOTAL_FLEET AS [HA], 
				HL * 100.0 / TOTAL_FLEET AS [HL], 
				LL * 100.0 / TOTAL_FLEET AS [LL], 
				NC * 100.0 / TOTAL_FLEET AS [NC], 
				PL * 100.0 / TOTAL_FLEET AS [PL], 
				TC * 100.0 / TOTAL_FLEET AS [TC], 
				WS * 100.0 / TOTAL_FLEET AS [WS]
			FROM 
				#TMP_TREND 
			ORDER BY 
				REP_DATE
		...
		BEGIN
			...
			BEGIN
				SELECT 
					...
					DATENAME(m, STR(SUBSTRING(CONVERT(VARCHAR(8),dim_calendar_id),5,2)) + '/1/2012') + ' - week ' +
					REP_WEEK_OF_YEAR AS [REP_WEEK_OF_YEAR],
					AVG(CARSALES) * 100.0 / AVG(TOTAL_FLEET) AS [CARSALES], 
					AVG(CARHOLD) * 100.0 / AVG(TOTAL_FLEET) AS [CARHOLD], 
					AVG(CU) * 100.0 / AVG(TOTAL_FLEET) AS [CU], 
					AVG(HA) * 100.0 / AVG(TOTAL_FLEET) AS [HA], 
					AVG(HL) * 100.0 / AVG(TOTAL_FLEET) AS [HL], 
					AVG(LL) * 100.0 / AVG(TOTAL_FLEET) AS [LL], 
					AVG(NC) * 100.0 / AVG(TOTAL_FLEET) AS [NC], 
					AVG(PL) * 100.0 / AVG(TOTAL_FLEET) AS [PL], 
					AVG(TC) * 100.0 / AVG(TOTAL_FLEET) AS [TC], 
					AVG(WS) * 100.0 / AVG(TOTAL_FLEET) AS [WS]
				FROM 
					#TMP_TREND 
				GROUP BY 
					REP_YEAR, REP_WEEK_OF_YEAR , SUBSTRING(CONVERT(VARCHAR(8),dim_calendar_id),5,2)
