...
-- =============================================
CREATE procedure [dbo].[spReportKPIUnavailableOperationalFleet]
...
--=====================================================================================	
	...
	BEGIN
		...
		(
			...
			REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, SHOP, DEFLEET, FS, RL, RP, TN,
			TOTAL_FLEET
		)
		...
		SELECT 
			...
			SUM(BD + MM + TW) AS 'SHOP', 
			SUM(TB + WS) AS 'DEFLEET', 
			SUM(FS) AS 'FS', 
			SUM(RL) AS 'RL', 
			SUM(RP) AS 'RP', 
			SUM(TN) AS 'TN',
		...
		SELECT @entity, @key1, 'Insert KPI Unavailable Operational Fleet', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')		
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		SELECT 
			...
			rowtype = 'W' ,
			d.REP_YEAR, d.REP_MONTH, d.Rep_WeekOfYear , DATEADD(d, 0, DATEDIFF(d, 0, d.REP_DATE)),
			SUM(BD + MM + TW)/MAX(NumDays) AS 'SHOP', 
			SUM(TB + WS)/MAX(NumDays) AS 'DEFLEET', 
			SUM(FS)/MAX(NumDays) AS 'FS', 
			SUM(RL)/MAX(NumDays) AS 'RL', 
			SUM(RP)/MAX(NumDays) AS 'RP', 
			SUM(TN)/MAX(NumDays) AS 'TN',
	...
	BEGIN
		...
		BEGIN
			SELECT 
				...
				REP_WEEK_OF_YEAR, 
				REP_DATE, 
				SHOP * 100.0 / TOTAL_FLEET AS [SHOP], 
				DEFLEET * 100.0 / TOTAL_FLEET AS [DEFLEET], 
				FS * 100.0 / TOTAL_FLEET AS [FS], 
				RL * 100.0 / TOTAL_FLEET AS [RL], 
				RP * 100.0 / TOTAL_FLEET AS [RP], 
				TN * 100.0 / TOTAL_FLEET AS [TN]
			FROM 
				#TMP_TREND ORDER BY REP_DATE
		END
		ELSE 
		BEGIN
			...
			BEGIN
				SELECT 
					...
					DATENAME(m, STR(SUBSTRING(CONVERT(VARCHAR(8),dim_calendar_id),5,2)) + '/1/2012') + ' - week ' +
					REP_WEEK_OF_YEAR AS [REP_WEEK_OF_YEAR],
					AVG(SHOP) * 100.0 / AVG(TOTAL_FLEET) AS [SHOP], 
					AVG(DEFLEET) * 100.0 / AVG(TOTAL_FLEET) AS [DEFLEET], 
					AVG(FS) * 100.0 / AVG(TOTAL_FLEET) AS [FS], 
					AVG(RL) * 100.0 / AVG(TOTAL_FLEET) AS [RL], 
					AVG(RP) * 100.0 / AVG(TOTAL_FLEET) AS [RP], 
					AVG(TN) * 100.0 / AVG(TOTAL_FLEET) AS [TN]
				FROM 
					#TMP_TREND 
				GROUP BY 
					REP_YEAR, REP_WEEK_OF_YEAR , SUBSTRING(CONVERT(VARCHAR(8),dim_calendar_id),5,2)
