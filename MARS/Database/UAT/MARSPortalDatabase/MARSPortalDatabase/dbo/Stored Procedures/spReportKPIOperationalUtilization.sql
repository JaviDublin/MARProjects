CREATE procedure [dbo].[spReportKPIOperationalUtilization]
...
/*
exec spReportKPIOperationalUtilization
		@start_date = '20120301'
		, @end_date = '20120310'
		, @country = 'GE'
...
--===========================================================
	...
	BEGIN
		...
		(
			...
			REP_YEAR, REP_MONTH, REP_WEEK_OF_YEAR, REP_DATE, ON_RENT, OPERATIONAL_FLEET
		)
		...
		SELECT @entity, @key1, 'Insert KPI Operational Utilization', 'updated=' + COALESCE(CONVERT(VARCHAR(20),@rowcount),'null')
	END
	ELSE
	BEGIN
		INSERT INTO #TMP_TREND
		...
		SELECT 
			...
			SUM(OPERATIONAL_FLEET)/MAX(NumDays) AS 'OPERATIONAL_FLEET' 
		FROM 
...
--===========================================================		
	...
	BEGIN
		...
		CASE
			 WHEN OPERATIONAL_FLEET = 0 THEN 0
			 ELSE (ON_RENT * 100.0) / OPERATIONAL_FLEET
		END AS [KPI]
		FROM 
			#TMP_TREND 
		ORDER BY 
	...
	BEGIN
		...
		BEGIN
			SELECT 
				...
				CASE
					WHEN AVG(OPERATIONAL_FLEET) = 0 THEN 0
					ELSE (AVG(ON_RENT)*100.0) / AVG(OPERATIONAL_FLEET)
				END AS [KPI]
			FROM 
				#TMP_TREND 
			GROUP BY 
